...
object HardwareMonitors {
    ...
    // 🚀 PHASE 3: Read Raw Kernel Nodes for Charging Wattage
    fun readChargingWattage(): Float {
        return try {
            ...
            // Nodes usually output in microvolts/microamps
            ...
            if (watts > 120f || watts < 0f) 0f else watts // Sanity check limits
        } catch (e: Throwable) {
            0f
        }
    }
    ...
    }.flowOn(Dispatchers.IO) // 🚀 BULLETPROOF FIX: Forces disk I/O completely off the SystemUI Main Thread
}
